...
class PSEPostProcess(object):
    ...
    def __call__(self, outs_dict, shape_list):
        ...
        kernels = (pred > self.thresh).float()
        text_mask = kernels[:, 0:1, :, :]     # batch * 1 * H * W
        kernels[:, 0:, :, :] = kernels[:, 0:, :, :] * text_mask     # batch * kernel_num-1 * H * W
    ...
    def boxes_from_bitmap(self, score, kernels, shape):
        label = pse(kernels, self.min_area / (self.scale ** 2))
